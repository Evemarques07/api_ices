...
@router.post("/", response_model=usuarios_schemas.Usuario)
...
@router.get("/", response_model=List[usuarios_schemas.Usuario])
def read_usuarios(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    usuarios_list = usuarios_crud.get_usuarios(db, skip=skip, limit=limit)
    return usuarios_list

@router.get("/{usuario_id}", response_model=usuarios_schemas.Usuario)
...
@router.put("/{usuario_id}", response_model=usuarios_schemas.Usuario)
